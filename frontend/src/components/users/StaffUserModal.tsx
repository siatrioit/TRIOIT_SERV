...
export function StaffUserModal({
  ...
  const { data: loadedUser } = useQuery({
    queryKey: ['user-detail', initial?.id],
    queryFn: () => usersApi.get(initial!.id),
  ...
  useEffect(() => {
    if (loadedUser?.data?.signature_data !== undefined) {
      setSignatureData(loadedUser.data.signature_data ?? null);
    }
  }, [loadedUser]);
        ...
        {mode === 'edit' && (
          <>
            ...
            <div className="pt-2 border-t border-gray-100">
              <p className="text-sm font-medium text-gray-700 mb-2">Paraksts PDF aktiem</p>
              <StaffSignaturePad
                value={signatureData}
