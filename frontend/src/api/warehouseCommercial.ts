...
export const warehouseCommercialApi = {
  ...
  listProducts: (params?: { search?: string; group_id?: string; exact?: boolean }) => {
  ...
  listReceipts: (params?: ListReceiptsParams) => {

    const q = new URLSearchParams();

    if (params?.supplier_id) q.set('supplier_id', params.supplier_id);

    if (params?.unpaid_only) q.set('unpaid_only', '1');
    ...
    return api.get<{ data: WarehouseReceipt[] }>(`/warehouse/receipts${qs ? `?${qs}` : ''}`);
  ...
  ) => api.put<{ data: WarehouseReceipt }>(`/warehouse/receipts/${id}`, data),
  ...
  ) => api.put<{ data: WarehouseReceipt }>(`/warehouse/receipts/${id}/lines`, { lines }),

  postReceipt: (id: string) => api.post<{ data: WarehouseReceipt }>(`/warehouse/receipts/${id}/post`),

  unpostReceipt: (id: string) =>

    api.post<{ data: WarehouseReceipt }>(`/warehouse/receipts/${id}/unpost`),

  payReceipt: (id: string, amount: number) =>

    api.post<{ data: WarehouseReceipt }>(`/warehouse/receipts/${id}/pay`, { amount }),
