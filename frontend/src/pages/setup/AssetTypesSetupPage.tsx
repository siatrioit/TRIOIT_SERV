...
export function AssetTypesSetupPage() {
  ...
  const updateComponentMutation = useMutation({
    mutationFn: ({ id, name }: { id: string; name: string }) =>
      assetTypesApi.updateComponent(id, { name }),
    onSuccess: invalidate,
    onError: (err) => setError(formatError(err)),
  });
        ...
        <div className="space-y-3">
          {types.map((type) => (
            <AssetTypeCard
              ...
              onRename={(name) => updateTypeMutation.mutate({ id: type.id, name })}
              onToggleActive={() =>
              ...
              onRenameComponent={(componentId, name) =>
                updateComponentMutation.mutate({ id: componentId, name })
              }
...
function AssetTypeCard({
  ...
  const [typeNameDraft, setTypeNameDraft] = useState(type.name);
  ...
  const startEditComponent = (component: AssetTypeComponent) => {
    setEditingComponentId(component.id);
    setComponentNameDraft(component.name);
  };

  const saveComponentName = (component: AssetTypeComponent) => {
    const name = componentNameDraft.trim();
    if (!name || name === component.name) {
      setEditingComponentId(null);
      return;
    }
    onRenameComponent(component.id, name);
    setEditingComponentId(null);
  };
          ...
          {editingType ? (
            <div className="flex gap-2 items-center">
              <input
                ...
                autoFocus
              />
              <button type="button" className="btn-primary !py-1.5 !px-3 text-xs" onClick={saveTypeName}>
                Saglabāt
              </button>
              <button
                type="button"
            ...
            <button type="button" className="text-left w-full" onClick={onToggleExpand}>
                ...
                >
                  {editingComponentId === c.id ? (
                    <div className="flex gap-2 items-center flex-1 min-w-0">
                      ...
                      <button
                        type="button"
                        className="text-xs px-2 py-1 rounded bg-primary-600 text-white shrink-0"
                        onClick={() => saveComponentName(c)}
                      >
                        OK
                      </button>
                      <button
                        type="button"
                        className="text-xs px-2 py-1 rounded bg-gray-100 shrink-0"
