...
async function buildIncidentScopeClause(grants) {
    ...
    if (objectScopeIds.length === 1) {
        ...
        parts.push('EXISTS (SELECT 1 FROM units u WHERE u.id = i.unit_id AND u.object_id = ?)');
        params.push(objectId);
    }
    else {
        ...
        parts.push(`EXISTS (SELECT 1 FROM units u WHERE u.id = i.unit_id AND u.object_id IN (${placeholders}))`);
        params.push(...objectScopeIds);
    }
